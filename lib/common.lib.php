...
function find_empty_slot($mb_id) {
    global $g5;
    ...
    // 너비 우선 탐색
    while (!empty($queue)) {
        $current_id = array_shift($queue); // 대기열에서 현재 노드 꺼내기

        // 현재 노드 정보 가져오기
        $member = sql_fetch("SELECT * FROM {$g5['member_table']} WHERE mb_id = '{$current_id}'");
        if (!$member) continue;

        // 빈 자리가 있는지 확인
        if (!$member['left_user_id']) {
            return ['parent_id' => $current_id, 'position' => 'left_user_id'];
        }
        if (!$member['right_user_id']) {
            return ['parent_id' => $current_id, 'position' => 'right_user_id'];
        }

        // 자식 노드를 대기열에 추가
        $queue[] = $member['left_user_id'];
        $queue[] = $member['right_user_id'];
    }
...
function get_content($content_id){
    global $g5;
    // 내용 가져오기
    $sql = "SELECT * FROM `g5_content` WHERE `co_id` = '$content_id'";
    $row = sql_fetch($sql);
