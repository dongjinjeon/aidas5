...
?>
<style>
    .member-list-btn-group .btn{width: 100px}
        ...
        <table>
            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    $s_grp = '<a href="./member_list_recommend.php?mb_id=' . $row['mb_id'] . '" class="btn btn_02">추천인</a>';
                ...
                ?>
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td headers="mb_list_id" class="td_name sv_use"><?php echo $mb_id ?>(<?=$row['mb_recommend'] ?>)</td>
                        <td headers="mb_list_name" class="td_mbname"><?php echo get_text($row['mb_name']); ?></td>
                        <td headers="mb_list_mobile" class="td_tel"><?php echo get_text($row['mb_hp']); ?></td>
                        <td headers="mb_list_lastcall" class="td_date"><?php echo substr($row['mb_today_login'], 2, 8); ?></td>
                        <td headers="mb_list_join" class="td_date"><?php echo substr($row['mb_datetime'], 2, 8); ?></td>
                        <td headers="mb_list_mng" class="td_mng td_mng_s member-list-btn-group" style="padding: 15px">
                            <?php if($row['mb_level'] == 1){?>
                            <button type="button" onclick="approve('<?=$row['mb_id'] ?>')" class="btn btn_01">승인</button>
                            <?php } ?>
                            <?php echo $s_grp ?>
                            <?php echo $s_mod ?>
                        </td>
...
<div class="loading-overlay" style="display: none;">
    <div class="loading-container">
        <img src="/img/loading.gif?v=2" alt="Loading" class="loading-icon">
    </div>
</div>
<script>
    ...
    function approve(mb_id){
      if(confirm("해당회원을 승인처리하시겠습니까?")){
          $(".loading-overlay").show();
          $.post("/api/index.php?p=member&m=approve",{
              mb_id : mb_id
          },function (data) {
              if($.trim(data) == "Y"){
                  alert("승인되었습니다.");
