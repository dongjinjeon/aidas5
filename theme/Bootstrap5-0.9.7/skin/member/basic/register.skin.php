...
<form name="fregister" action="<?php echo $register_action_url ?>" onsubmit="return fregister_submit(this);" method="POST" autocomplete="off">
	...
	<div id="default">
        ...
        <div class="mb-4">
            <div class="form-check mb-2">
                <input type="checkbox" id="agree31" name="agree3" value="1" class="form-check-input">
                <label class="form-check-label" for="agree31">개인정보 수집ㆍ이용</label>
            </div>
            <textarea class="form-control" rows="5" style="font-size: 0.8rem;"><?php echo get_text($config['cf_collection']) ?></textarea>
        </div>
...
<script>
	function fregister_submit(f)
	{
        ...
        if (!f.agree3.checked) {
            alert("개인정보 수집ㆍ이용 내용에 동의하셔야 회원가입 하실 수 있습니다.");
            f.agree3.focus();
            return false;
        }
